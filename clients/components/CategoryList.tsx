...
import { Group, Box, Collapse, Text, UnstyledButton, Container } from '@mantine/core';
import { 
  IconChevronRight, 
  IconFish,
  IconCoffee,
      ...
      <UnstyledButton onClick={() => setOpened((o) => !o)} className="block">
        <Group justify="space-between" gap={10}>
          ...
          {hasLinks && (
            <IconChevronRight
              stroke={1.5}
              className={`transition-transform ease-[150ms] w-[16px] h-[16px] ${opened ? 'transform-rotate-[-90deg]' : ''}`}
            />
          )}
        </Group>
      </UnstyledButton>
